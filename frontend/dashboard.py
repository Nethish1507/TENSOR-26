...
import threading
from backend.edge_inference import EdgeNodeClassifier
import random

# Global state for shared alerts (optional, but helps with "Command Center" feel)
if 'shared_alerts' not in st.session_state:
    st.session_state.shared_alerts = []

class CloudSimulator:
    def __init__(self):
        self.edge_node = EdgeNodeClassifier()
        self.base_lat, self.base_lon = 44.5, -110.5

    def random_coordinate(self):
        return [self.base_lat + random.uniform(-0.15, 0.15), self.base_lon + random.uniform(-0.15, 0.15)]

    def generate_step(self):
        event_type = random.choice(["audio", "gps", "audio", "audio", "gps"])
        if event_type == "audio":
            audio_choice = random.choice(["gun", "chainsaw", "wildlife", "wind"])
            result = self.edge_node.analyze_audio(f"virtual_fs/{audio_choice}.wav")
            coord = self.random_coordinate()
        else:
            result = self.edge_node.analyze_gps_anomaly({"status": "anomaly_movement"})
            coord = self.random_coordinate()
...
def run_simulation_loop():
    sim = CloudSimulator()
    while True:
        alert = sim.generate_step()
        if 'shared_alerts' in st.session_state:
            st.session_state.shared_alerts.insert(0, alert)
            st.session_state.shared_alerts = st.session_state.shared_alerts[:25]
        time.sleep(random.uniform(5, 12))

# Start background thread if not already running
if 'simulator_started' not in st.session_state:
    # We only start the simulator if we are in cloud mode or if the user wants it
    # For simplicity in this deployment, we'll start it for any session that doesn't see a file
...
def main():
    ...
    def get_alerts():
        # If we have local file, use it. Otherwise use the session simulator.
        if os.path.exists(ALERTS_FILE):
            try:
                with open(ALERTS_FILE, 'r') as f:
                    return json.load(f)
            except:
                return st.session_state.get('shared_alerts', [])
        return st.session_state.get('shared_alerts', [])
